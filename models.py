...
class TrAISformer(nn.Module):
    ...
    def __init__(self, config, partition_model = None):
        ...
        # Auxiliary features
        self.use_port = getattr(config, 'use_port_features', False)
        self.use_coast = getattr(config, 'use_coast_features', False)
        self.predict_ports = getattr(config, 'predict_ports', False)
        self.port_prediction_weight = getattr(config, 'port_prediction_weight', 0.1)
        
        if self.use_port:
            self.port_size = config.port_size
            self.n_port_embd = config.n_port_embd
            self.port_emb = nn.Embedding(self.port_size, config.n_port_embd)
            
            # NEW: Temporal port prediction head
            if self.predict_ports:
                self.port_prediction_head = nn.Sequential(
                    nn.Linear(config.n_embd, config.n_embd // 2),
                    nn.GELU(),
                    nn.Dropout(config.resid_pdrop),
                    nn.Linear(config.n_embd // 2, self.port_size)
                )
            
        if self.use_coast:
            self.coast_size = config.coast_size
            self.n_coast_embd = config.n_coast_embd
            self.coast_emb = nn.Embedding(self.coast_size, config.n_coast_embd)
    ...
    def to_indexes(self, x, mode="uniform"):
        ...
        x_base = x[:, :, :4]
        if mode == "uniform":
            idxs = (x_base*self.att_sizes).long()
            return idxs, idxs
...
#             pdb.set_trace()
    ...
    def forward(self, x, masks = None, with_targets=False, return_loss_tuple=False, port_labels=None):
        """
        Args:
            x: a Tensor of size (batchsize, seqlen, 4). x has been truncated 
                to [0,1).
        ...
        # Separate base and auxiliary features
        base_x = x[:, :, :4]  # lat, lon, sog, cog
        
        if self.mode in ("mlp_pos", "mlp"):
            idxs, idxs_uniform = base_x, base_x
        else:
            idxs, idxs_uniform = self.to_indexes(base_x, mode=self.partition_mode)
        
        if with_targets:
            inputs = idxs[:, :-1, :].contiguous()
            targets = idxs[:, 1:, :].contiguous()
            targets_uniform = idxs_uniform[:, 1:, :].contiguous()
            x_aux = x[:, :-1, 4:].contiguous()
            
            # NEW: Shift port labels too
            if port_labels is not None:
                port_labels_shifted = port_labels[:, 1:].contiguous()
            else:
                port_labels_shifted = None
        else:
            inputs = idxs
            x_aux = x[:, :, 4:]
            targets = None
        ...
        # Create embeddings for base features
        ...
        embedding_list = [lat_embeddings, lon_embeddings, 
                         sog_embeddings, cog_embeddings]
        
        # Add auxiliary embeddings
        aux_idx = 0
        if self.use_port:
            port_idxs = (x_aux[:, :, aux_idx] * self.port_size).long()
            port_idxs = torch.clamp(port_idxs, 0, self.port_size - 1)
            port_embeddings = self.port_emb(port_idxs)
            embedding_list.append(port_embeddings)
            aux_idx += 1
            
        if self.use_coast:
            coast_idxs = (x_aux[:, :, aux_idx] * self.coast_size).long()
            coast_idxs = torch.clamp(coast_idxs, 0, self.coast_size - 1)
            coast_embeddings = self.coast_emb(coast_idxs)
            embedding_list.append(coast_embeddings)
        
        token_embeddings = torch.cat(embedding_list, dim=-1)
        
        # Transformer processing
        position_embeddings = self.pos_emb[:, :seqlen, :]
        fea = self.drop(token_embeddings + position_embeddings)
        fea = self.blocks(fea)
        fea = self.ln_f(fea)  # (batch, seq, n_embd)
        ...
        # NEW: Port prediction at every timestep
        port_probs = None
        port_loss = None
        if self.predict_ports and self.use_port:
            port_logits = self.port_prediction_head(fea)
            port_probs = F.softmax(port_logits, dim=-1)
            
            # Calculate port prediction loss if labels provided
            if port_labels_shifted is not None and targets is not None:
                port_loss = F.cross_entropy(
                    port_logits.reshape(-1, self.port_size),
                    port_labels_shifted.reshape(-1),
                    reduction='none',
                    ignore_index=-100  # <--- CRITICAL ADDITION
        ...
        if targets is not None:
            ...
            # NEW: Add port prediction loss
            if port_loss is not None:
                loss = loss + self.port_prediction_weight * port_loss
        ...
        if self.predict_ports:
            if return_loss_tuple:
                return logits, loss, loss_tuple, port_probs
            else:
                return logits, loss, port_probs
        else:
            if return_loss_tuple:
